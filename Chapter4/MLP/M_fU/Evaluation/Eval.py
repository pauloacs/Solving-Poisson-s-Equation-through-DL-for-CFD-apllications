...
class Evaluation():
	def __init__(self, delta, shape, avance, var_p, var_in, hdf5_path, model_directory):
		...
		self.max_abs_fU, self.max_abs_dist, self.max_abs_p = maxs[0], maxs[1], maxs[2]
		...
		self.pc_p = np.argmax(self.pcap.explained_variance_ratio_.cumsum() > self.var_p) if np.argmax(self.pcap.explained_variance_ratio_.cumsum() > self.var_p) > 1 and np.argmax(self.pcap.explained_variance_ratio_.cumsum() > self.var_p) <= 512 else 512  #max defined to be 32 here
		self.pc_in = np.argmax(self.pcainput.explained_variance_ratio_.cumsum() > self.var_in) if np.argmax(self.pcainput.explained_variance_ratio_.cumsum() > self.var_in) > 1 and np.argmax(self.pcainput.explained_variance_ratio_.cumsum() > self.var_in) <= 512 else 512
		print(self.pc_p)
		print(self.pc_in)
	...
	def timeStep(self, sim, time, save_plots, show_plots, apply_filter):
			...
			fU = data[i,j,:self.indice,5:6] #values
			...
			fU_interp = self.interpolate_fill(fU_adim, self.vert, self.weights)#takes virtually no time  because "vert" and "weigths" where already calculated

			grid = np.zeros(shape=(1, self.grid_shape_y, self.grid_shape_x, 4))

			grid[0,:,:,0:1][tuple(self.indices.T)] = fU_interp.reshape(fU_interp.shape[0],1)
			grid[0,:,:,1:2] = self.sdfunct
			grid[0,:,:,2:3][tuple(self.indices.T)] = p_interp.reshape(p_interp.shape[0],1)

			grid[np.isnan(grid)] = 0 #set any nan value to 0

			grid[0,:,:,0:1] = grid[0,:,:,0:1]/self.max_abs_fU
			grid[0,:,:,1:2] = grid[0,:,:,1:2]/self.max_abs_dist
			grid[0,:,:,2:3] = grid[0,:,:,2:3]/self.max_abs_p
		...
		#	plt.show()
			...
			if save_plots:
				...
				masked_arr = np.ma.array(result_array[0,:,:,0], mask=(grid[0,:,:,1] == 0))
				...
				masked_arr = np.ma.array(grid[0,:,:,2], mask=(grid[0,:,:,1] == 0))
				...
				masked_arr = np.ma.array( np.abs(( grid[0,:,:,2] -result_array[0,:,:,0] )/(np.max(grid[0,:,:,2]) -np.min(grid[0,:,:,2]))*100) , mask=(grid[0,:,:,1] == 0))
