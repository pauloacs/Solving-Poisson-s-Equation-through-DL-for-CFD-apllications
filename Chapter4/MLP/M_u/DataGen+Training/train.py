...
def read_dataset(path, path_rect,print_shape, delta, Nsamples, block_size):

    global num_sims
    x_data = []
    hdf5_file = h5py.File(path, "r")
    data = hdf5_file["sim_data"][:num_sims, :num_times, ...]
    top_boundary = hdf5_file["top_bound"][:num_sims, :num_times, 
    ...]
    obst_boundary = hdf5_file["obst_bound"][:num_sims, :num_times, ...]
    hdf5_file.close()

    x_data1 = []
    hdf5_file = h5py.File(path_rect, "r")
    data_rect = hdf5_file["sim_data"][:num_sims_rect, :num_times, ...]
    top_boundary_rect = hdf5_file["top_bound"][:num_sims_rect, :num_times,
    ...]
    obst_boundary_rect = hdf5_file["obst_bound"][:num_sims_rect, :num_times, ...]
    hdf5_file.close()

    hdf5_file = h5py.File(path_tria, "r")
    data_tria = hdf5_file["sim_data"][:num_sims_tria, :num_times, ...]
    top_boundary_tria = hdf5_file["top_bound"][:num_sims_tria, :num_times,
    ...]
    obst_boundary_tria = hdf5_file["obst_bound"][:num_sims_tria, :num_times, ...]
    hdf5_file.close()

    hdf5_file = h5py.File(path_placa, "r")
    data_placa = hdf5_file["sim_data"][:num_sims_tria, :num_times, ...]
    top_boundary_placa = hdf5_file["top_bound"][:num_sims_tria, :num_times,
    ...]
    obst_boundary_placa = hdf5_file["obst_bound"][:num_sims_tria, :num_times, ...]
    hdf5_file.close()
    ...
    for i in range(data.shape[0] + data_rect.shape[0] + data_tria.shape[0] + data_placa.shape[0]):

        if i >= num_sims_rect and i < (num_sims_rect+num_sims) :
          i = int(i-num_sims_rect)
          sim_cil = True
          indice = index(data[i,0,:,0] , -100.0 )[0]
          data_limited = data[i,0,:indice,:]#[mask_x]
        elif i>= (num_sims_rect+num_sims) and i< (num_sims_rect+num_sims+num_sims_tria):
          i = int(i-num_sims_rect-num_sims) 
          sim_cil = False
          sim_tria = True
          indice = index(data_tria[i,0,:,0] , -100.0 )[0]
          data_limited = data_tria[i,0,:indice,:]#[mask_x]
        elif i>= (num_sims_rect+num_sims+num_sims_tria):
          i = int(i-num_sims_rect-num_sims-num_sims_tria)
          sim_cil = False
          sim_tria = False
          sim_placa = True
          indice = index(data_placa[i,0,:,0] , -100.0 )[0]
          data_limited = data_placa[i,0,:indice,:]#[mask_x]
        else:
          indice = index(data_rect[i,0,:,0] , -100.0 )[0]
          data_limited = data_rect[i,0,:indice,:]#[mask_x]
        ...
        if sim_cil :
          ...
          domain_bool, sdf = domain_dist(i, top_boundary_tria, obst_boundary_tria, xy0)
        elif sim_placa:
          domain_bool, sdf = domain_dist(i, top_boundary_placa, obst_boundary_placa, xy0)
        else:
          domain_bool, sdf = domain_dist(i, top_boundary_rect, obst_boundary_rect, xy0)
        ...
        for j in range(data.shape[1]):  #100 for both data and data_rect
          ...
          if sim_cil :
            ...
            data_limited = data_tria[i,j,:indice,:]#[mask_x]
          elif sim_placa:
            data_limited = data_placa[i,j,:indice,:]#[mask_x]
          else:
            data_limited = data_rect[i,j,:indice,:]#[mask_x]
          ...
          Ux_interp = interpolate_fill(Ux_adim, vert, weights)#takes virtually no time  because "vert" and "weigths" where already calculated
          Uy_interp = interpolate_fill(Uy_adim, vert, weights)
          ...
          grid[0,:,:,2:3] = sdfunct
          grid[0,:,:,3:4][tuple(indices.T)] = p_interp.reshape(p_interp.shape[0],1)
          ...
          for [jj, ii] in XY_indices:
                  ...
                  y_list.append(grid[0, i_range[0]:i_range[1] , j_range[0]:j_range[1] , 3:4 ])
    ...
    max_abs_Ux = np.max(np.abs(max_abs_Ux))
    max_abs_Uy = np.max(np.abs(max_abs_Uy))
    max_abs_dist = np.max(np.abs(max_abs_dist))
    max_abs_p = np.max(np.abs(max_abs_p))

    print(max_abs_Ux, max_abs_Uy, max_abs_dist, max_abs_p)

    np.savetxt('maxs', [max_abs_Ux, max_abs_Uy, max_abs_dist, max_abs_p] )

    split = 0.9
    len_train = int(split*x_array.shape[0])
        
    return max_abs_Ux, max_abs_Uy, max_abs_dist, max_abs_p, len_train
...
def prepare_data (block_size, Nsamples, num_principal_comp ):
  ...
  print_shape = True
  max_abs_Ux, max_abs_Uy, max_abs_dist, max_abs_p, len_train = read_dataset(hdf5_path, path_rect ,print_shape, delta, Nsamples, block_size)
  ...
  N = int(Nsamples * (num_sims+num_sims_rect+num_sims_tria+num_sims_placa))

  chunk_size = int(N/500)
  ...
  for i in range(int(N//chunk_size)):
    ...
    if x_array.shape[0] < modes_PCA:
      print('chunck too small ... , skipping')
      break

    x_array_flat = x_array.reshape((x_array.shape[0], x_array.shape[1]*x_array.shape[2], 2 ))
    x_array_flat1 = x_array_flat[...,0:1]/max_abs_Ux
    x_array_flat2 = x_array_flat[...,1:2]/max_abs_Uy
  ...
  print('PC_p :' + str(PC_p), flush = True)
  print('PC_input :' + str(PC_input), flush = True)
  ...
  for i in range(int(N//chunk_size)):
    ...
    input_transf = ipca_input.transform(input_flat)[:,:PC_input]
    y_transf = ipca_p.transform(y_array_flat)[:,:PC_input]
...
def load_data_And_train(lr, Nsamples, block_size, delta, features, batch_size, num_principal_comp, model_name, hdf5_path, beta_1, num_epoch):
  ...
  # lrs = 1e-7 * (10 ** (np.arange(100) / 20)) # to study lr
  # plt.semilogx(lrs, epochs_train_losses)
  # plt.axis([1e-7, 1e-2, 0, 300])
  # plt.show()

  # epochs = np.arange(10)
  # plt.semilogx(epochs, epochs_train_losses)
  # plt.semilogx(epochs, epochs_val_losses)
  # plt.plot(epochs, epochs_train_losses)
  # plt.plot(epochs, epochs_val_losses)
...
path_rect = 'dataset_unsteadyRect_fu_bound.hdf5' #adjust path
path_tria = 'dataset_unsteadyTriangle_fu_bound.hdf5'
path_placa = 'dataset_unsteadyPlaca_fu_bound.hdf5'


# DATA-Processing PARAMETERS

Nsamples = int(2e6/(10+12+12+14))
block_size = 128
delta = 5e-3
modes_PCA = 512
...
len_train = int(0.9*Nsamples*(num_sims+num_sims_rect))
...
load_data_And_train(lr, Nsamples, block_size, delta, filters, batch_size, modes_PCA ,model_name,hdf5_path,beta, num_epoch )
