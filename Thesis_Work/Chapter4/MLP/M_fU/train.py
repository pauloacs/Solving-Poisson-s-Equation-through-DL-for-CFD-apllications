...
class Training:
  ...
  def read_dataset(self):
      ...
      for i in range(self.numSimsCil +self.numSimsRect + self.numSimsTria + self.numSimsPlate):
          ...
          for j in range(data.shape[1]):  #100 for both data and data_rect
            ...
            p_interp = self.interpolate_fill(p_adim, vert, weights) #compared to the griddata interpolation 
      ...
      self.max_abs_fU = np.max(np.abs(max_abs_fU))
      self.max_abs_dist = np.max(np.abs(max_abs_dist))
      self.max_abs_p = np.max(np.abs(max_abs_p))

      np.savetxt('maxs', [self.max_abs_fU, self.max_abs_dist, self.max_abs_p] )
  ...
  def prepare_data (self, hdf5_paths, num_principal_comp ):
    ...
    for i in range(int(N//chunk_size)):
      ...
      x_array = f.root.data[i*chunk_size:(i+1)*chunk_size,:,:,0:1] # e.g. read from disk only this par$
      ...
      x_array_flat1 = x_array_flat[...,0:1]/self.max_abs_fU
      ...
      print('fitted ' + str(i) + '/' + str(N//chunk_size), flush = True)
...
def main():
  ...
  path_cil = '/home/paulo/dataset_unsteadyCil_fu_bound.hdf5' #adjust path
