...
def domain_dist(top_boundary, obst_boundary, xy0):
  ...
  top = top[0:top.shape[0]:step,:]   #if this has too many values, using cdist can crash the memory
  obst = obst[0:obst.shape[0]:step,:]   #hence, the step should be increased for large meshes
...
#	np.save('array.npy', array)
	...
	p[sdf_mesh < 0.05] = p_prev[sdf_mesh < 0.05] #to overwrite the regions near the objects
